class _Tooltip {
  ...
  constructor() {
    ...
    <div id="tooltip">
			...
			<div class="tooltip-enable" role="button" tabindex="2" style="user-select: none; transition: background 20ms ease-in 0s; cursor: pointer; display: flex; align-items: center; padding-left: 7px; padding-right: 6px; white-space: nowrap; box-shadow: rgba(255, 255, 255, 0.094) 1px 0px 0px; margin-right: 1px;"><div style="display: inline-flex; align-items: center; justify-content: center; height: 18px; text-align: center; font-size: 15px; border-radius: 2px; width: 48px; font-weight: 500; margin-bottom: 2px; color: rgba(255, 255, 255, 0.81);"><input type="color" style="appearance: none;background-color: transparent;border: none;inline-size: 0;block-size: 0;"><span>Color</span></div><svg role="graphics-symbol" viewBox="0 0 30 30" class="chevronDown" style="width: 10px; height: 100%; display: block; fill: white; flex-shrink: 0; margin-left: 3px;"><polygon points="15,17.4 4.8,7 2,9.8 15,23 28,9.8 25.2,7 "></polygon></svg></div>
			<div class="tooltip-enable" role="button" tabindex="3" style="user-select: none; transition: background 20ms ease-in 0s; cursor: pointer; display: flex; align-items: center; padding-left: 7px; padding-right: 6px; white-space: nowrap; box-shadow: rgba(255, 255, 255, 0.094) 1px 0px 0px; margin-right: 1px;"><div style="display: inline-flex; align-items: center; justify-content: center; height: 18px; text-align: center; font-size: 15px; border-radius: 2px; width: 48px; font-weight: 500; margin-bottom: 2px; color: rgba(255, 255, 255, 0.81);">Font</div><svg role="graphics-symbol" viewBox="0 0 30 30" class="chevronDown" style="width: 10px; height: 100%; display: block; fill: white; flex-shrink: 0; margin-left: 3px;"><polygon points="15,17.4 4.8,7 2,9.8 15,23 28,9.8 25.2,7 "></polygon></svg></div>
			<div class="tooltip-enable" role="button" tabindex="4" style="user-select: none; transition: background 20ms ease-in 0s; cursor: pointer; display: flex; align-items: center; padding: 0px 8px; color: rgba(255, 255, 255, 0.81);"><span style="font-weight: 600;">B</span></div>
			<div class="tooltip-enable" role="button" tabindex="5" style="user-select: none; transition: background 20ms ease-in 0s; cursor: pointer; display: flex; align-items: center; padding: 0px 8px; color: rgba(255, 255, 255, 0.81);"><span style="text-decoration: underline;">U</span></div>
			<div class="tooltip-enable" role="button" tabindex="6" style="user-select: none;transition: background 20ms ease-in 0s;cursor: pointer;display: flex;align-items: center;padding: 0px 8px;color: rgba(255, 255, 255, 0.81);box-shadow: rgba(255, 255, 255, 0.094) 1px 0px 0;"><span style="text-decoration: line-through;">S</span></div>
			<div class="tooltip-desable" role="button" tabindex="7" style="user-select: none;transition: background 20ms ease-in 0s;cursor: pointer;display: flex;align-items: center;padding: 0px 8px;color: rgba(255, 255, 255, 0.81);box-shadow: rgba(255, 255, 255, 0.094) 1px 0px 0;"><img src="./loading-ai-2.gif" style="width: 15px; height: 15px;"/></div>
  ...
  setSelection(value) {
    ...
    if (this.spanSelection != undefined) {
      this.underline.style.color = this.spanSelection.style.textDecoration == "underline" ? "deepskyblue" : "rgba(255, 255, 255, 0.81)";
      this.bold.style.color = this.spanSelection.style.fontWeight == "bold" ? "deepskyblue" : "rgba(255, 255, 255, 0.81)";
      this.strikethrough.style.color = this.spanSelection.style.textDecoration == "line-through" ? "deepskyblue" : "rgba(255, 255, 255, 0.81)";
      this.inputColor.value = this.spanSelection.style.color;
    } else {
      this.underline.style.color = "rgba(255, 255, 255, 0.81)";
      this.bold.style.color = "rgba(255, 255, 255, 0.81)";
      this.strikethrough.style.color = "rgba(255, 255, 255, 0.81)";
      this.inputColor.type = "color";
      this.inputColor.value = "rgba(255, 255, 255, 0.81)";
    }
  }
  ...
  show(text, rect, selection, startElement, endElement) {
    ...
    for (let i = 0; i < selection.rangeCount; i++) {
      ...
      for (let i = 0; i < elementsAll.length; i++) {
        ...
        if (isStart) elements.push(elementsAll[i]);
  ...
  updateTextAll(color, font, underline, bold, strikethrough, italic, comment) {
    ...
    if (color != undefined) color = baseColor == undefined ? "rgba(255, 255, 255, 0.81)" : color;
  ...
  updateText(selection, element, elementSpan, color, font, underline, bold, strikethrough, italic, comment) {
    ...
    if (selection.elementSpan == undefined && (elementSpan == undefined || selection.isTextComplete == false)) {
      ...
      element.innerHTML = `${htmlStart}${span.outerHTML}${htmlEnd}`;
  ...
  getParentDetails(paragraph) {
    if (paragraph.tagName == "P" && paragraph.parentElement.tagName == "SUMMARY") {
  ...
  createDetails(main_paragraph, response) {
    ...
    //  if (main_paragraph == undefined) return;
    //  if (main_paragraph.hasAttribute("edit-summary")) main_paragraph = this.getParentDetails(main_paragraph);
    //}
